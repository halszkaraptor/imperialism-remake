...
public final class StreamState {
    ...
    public int pagein(Page og) {
        ...
        int bos = og.bos();
        int eos = og.eos();
        long local_granulepos = og.granulepos();
        int _serialno = og.serialno();
        int _pageno = og.pageno();
        int segments = header_base[local_header + 26] & 0xff;
        ...
        if (_pageno != pageno) {
            ...
            if (continued != 0) {
                ...
                for (; segptr < segments; segptr++) {
                    int val = (header_base[local_header + 27 + segptr] & 0xff);
                    body += val;
                    bodysize -= val;
                    if (val < 255) {
                        segptr++;
...

        {
            ...
            if (saved != -1) {
                granule_vals[saved] = local_granulepos;
            }
        }
