//    $Id: GSMDecoder.java,v 1.5 2013/01/13 23:56:11 pfisterer Exp $	
...
public final class GSMDecoder
{
            ...
            throws InvalidGSMFrameException
    {
        ...
        {
        case TOAST:
            anDecodedData = decode(abFrame, nFrameStart, true);
            for (int i = 0; i < 160; i++)
            {
        ...
        case MICROSOFT:
            ...
            anDecodedData = decode(abFrame, nFrameStart + 33, false);
            for (int i = 0; i < 160; i++)
            {
                TConversionTool.intToBytes16(anDecodedData[i], abBuffer, i * 2
     ...
     *            frame is the second half of a MS frame. For toast frames the
     *            value of the parameter is ignored.
     * @return an array containing the decoded samples
     * @throws InvalidGSMFrameException
     */
    private final int[] decode(byte[] c, int bufferStartOffset,
            boolean firstHalfOfMicrosoftFrame) throws InvalidGSMFrameException
    {
        switch (gsmFrameFormat)
        {
        case TOAST:
